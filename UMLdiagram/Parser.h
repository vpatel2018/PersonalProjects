...
class Parser {
  ...
  private:
   ...
   void rearrange(vector<string> &list);
...
vector<vector<string> > Parser::aggregation(vector<string> list, vector<string> files) {
  ...
  for(int u = 0; u < extracts.size(); u++) {
    ...
    for(int i = 0; i < 33; i++) {
      ...
      for(int q = 0; q < line.length() - len + 1; q++) {
        if(line.substr(q, len) == "#" + keyword + "#") {
          extracts[u][1] = extracts[u][1].replace(q, len, "#");
        }
      }
    }
  }
  ...
  for(int x = 0; x < extracts.size(); x++) {
    ...
    for(int y = 1; y < extracts[x].size(); y++) {
      ...
      if(extracts[x][y].find("BREAKPOINT") != string::npos) {
        ...
        //check if there is an instance variable in Exact front for title/varname
        //if so, trim the instance variable name
        //then see if there are any list methods being invoked or not
        int amt = 0;
        ...
        //if there is no instance variable, then we check if self. keyword is there
        //if it is, remove it
        //if it is not, then make title a null thing
        ...
        //check if title or varname invokes any methods
        for(int w = 0; w < pieces.size(); w++) {
        ...
        //if no invocations are made, then make title a null thing
        if(amt == 0) {
          title = "NULL";
        }
        ...
        //now check the value to see whether it has an instance variable in EXACT front or not
        //if it does, remove it, check it if has list invocations, exit for loop
        for(int f = 0; f < variables.size(); f++) {
        ...
        //if value has self., remove the self. keyword
        ...
        //check to see if value has method invocations
        for(int w = 0; w < pieces.size(); w++) {
          for(int i = 1; i < pieces[w].size(); i++) {
        ...
        //if there are no method or list invocations, check to see if value is a class Object
        //if it is not, then make value a null thing
        if(amt == 0) {
          int times = 0;
          for(int q = 0; q < pieces.size(); q++) {
   ...
   for(int k = 0; k < list.size(); k++) {
	...
	if(trim(list[k]).find("_init_") != string::npos) {
      if(list[k].find("(") != string::npos) {
        ...
        if(posit != -1) {
          line += list[k];
          for(int e = k + 1; e <= posit; e++) {
            line += list[e].substr(firstCharPlace(list[e]), list[e].length() - firstCharPlace(list[e]));
          }
          list[k] = line;
          for(int u = k + 1; u <= posit; u++) {
        ...
        for(int h = 0; h < line.length(); h++) {
          if(line.at(h) == ',') {
            places.push_back(h-1);
            places.push_back(h+1);
          }
        }
        places.push_back(line.length() - 1);
        for(int q = 0; q < places.size(); q += 2) {
          string sub = line.substr(places[q], places[q+1] - places[q] + 1);
          if(trim(sub) != "self" && trim(sub) != "**kwargs") {
            string instance = "self." + sub;
            instance = instance.substr(0, instance.find("="));
            instance = trim(instance);
            acceptable.push_back(instance);
          }
        }
      }
	}
    ...
    for(int e = 0; e < list.size(); e++) {
      ...
      for(int k = 0; k < sub.size(); k++) {
        if(sub[k].find("import ") != string::npos && trim(sub[k]).find("import") == 0) {
          ...
          string line = trim(sub[k].substr(sub[k].find("from ") + 4, sub[k].length() - (sub[k].find("from ") + 4)));
          int i = -1;
          for(int d = 0; d < files.size(); d++) {
            if(line == files[d]) {
              list[e][k] = "IMPORT " + line + ".py";
...
void Parser::rearrange(vector<string> &list) {
  
    int position = -1;
    vector<string> copy;
    
    if(list[0].find("class ") != string::npos) {
      if(list[0].find("(") != string::npos) {
        list[0] = list[0].substr(0,list[0].find("("));
      }
      list[0] = list[0].replace(list[0].find("class "), 6, "");
    }
    
    for(int k = 0; k < list.size(); k++) {
        if(list[k].find("def") != string::npos) {
           position = k;
           break;
        }
    }
    ...
    if(position != -1) {
      ...
      for(int x = 0; x < position; x++) {
        if(list[x].find("=") != string::npos) {
          list[x] = list[x].substr(firstCharPlace(list[x]), list[x].find("=") - firstCharPlace(list[x]));
        }
        copy.push_back(list[x]);
      }
          
      for(int j = position; j < list.size(); j++) {
        if(list[j].find("self.") != string::npos && list[j].find("=") != string::npos) {
           list[j] = list[j].substr(firstCharPlace(list[j]), list[j].find("=") - firstCharPlace(list[j]));
           int c = 0;
           for(int y = j - 1; y >= 0; y = y - 1) {
             if(list[y] == list[j]) {
               c++;
             }
           }
      ...
      for(int j = position; j < list.size(); j++) {
        if(list[j].find("def") != string::npos && list[j].find(":") != string::npos) {
          list[j] = list[j].replace(list[j].find("def"), 3, "");
          list[j] = list[j].substr(firstCharPlace(list[j]), list[j].length() - firstCharPlace(list[j]));
          copy.push_back(list[j]);
        }
      }
