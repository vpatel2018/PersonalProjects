...
int getWidth(const char * word) {
    TTF_Font * t = TTF_OpenFont("Times New Roman.ttf", fontSize);
    int width;
    int height;
    int answer = 0;
...
int main(int argc, char * argv[]) {
    ...
    const int screenWidth = 1000; //set a screen width to any number you think is best
    const int screenHeight = 1000; //set a screen height to any number you think is best
    
    Graphics screen(screenWidth, screenHeight, fontSize);
    Parser p;
    vector<string> files;
    ...
    //1 = classname, 2 = x, 3 = y, 4 = w, 5 = h, 6 = row # --> storage
